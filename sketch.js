...
function preload() {
  ...
  SomFinal = loadSound("assets/background_music.mp3")
  SomTiro = loadSound("assets/cannon_explosion.mp3")
  SomAgua = loadSound("assets/cannon_water.mp3")
  SomPirata = loadSound("assets/pirate_laugh.mp3")
}
  ...
  if (boats.length > 0) {
    ...
    for (var i = 0; i < boats.length; i++) {
      ...
      if (collision.collided && !boats[i].isBroken) {
        isGameOver = true;
        gameOver();
        if (!isLaugh&&!SomPirata.isPlaying()){
          SomPirata.play()
           isLaugh = true
            SomPirata.setVolume(0.1)
        }
      }
    }
  } else {
